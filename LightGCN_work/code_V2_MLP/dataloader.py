...
class Loader(BasicDataset):
    ...
    Generic text dataset loader (amazon-book, gowalla, yelp2018, etc.)
    """

    def __init__(self, config=world.config, path=None):
        # Resolve dataset path from world.dataset unless user passed an explicit path
        if path is None:
            path = os.path.join("..", "data", world.dataset)
        self.path = path
        ...
        if not os.path.exists(train_file):
            raise FileNotFoundError(f"train.txt not found at: {train_file}")
        ...
        # ids are zero-based; +1 to get counts
        self.m_item += 1
        self.n_user += 1
    ...
    def _split_A_hat(self, A):
        ...
        for i_fold in range(self.folds):
            ...
            end = (self.n_users + self.m_items) if i_fold == self.folds - 1 else (i_fold + 1) * fold_len
    ...
    def getSparseGraph(self):
        ...
        if self.Graph is None:
            npz_path = os.path.join(self.path, 's_pre_adj_mat.npz')
            try:
    ...
    def __build_test(self):
        ...
        for u, i in zip(self.testUser, self.testItem):
            test_data.setdefault(int(u), []).append(int(i))
        return test_data
    ...
    def getUserPosItems(self, users):
        posItems = []
        for user in users:
            posItems.append(self.UserItemNet[int(user)].nonzero()[1])
        return posItems
