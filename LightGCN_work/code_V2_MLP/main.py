...
def run_training():
    ...
    def _ensure_csv_headers(train_csv, valid_csv, topks):
        ...
        if not os.path.exists(valid_csv):
            ...
            for k in topks: fields += [f"precision@{k}", f"recall@{k}", f"ndcg@{k}"]
    ...
    if resume_flag or resume_path:
        candidate = resume_path if (resume_path and os.path.exists(resume_path)) else (last_ckpt if os.path.exists(last_ckpt) else (weight_file if os.path.exists(weight_file) else None))
        if candidate:
            try:
    ...
    try:
        for epoch in range(start_epoch, world.TRAIN_epochs + 1):
            ...
            save_checkpoint(Recmodel, optimizer, epoch, best_metric=best_ndcg, scheduler=scheduler, path=os.path.join(world.FILE_PATH, 'last.pth.tar'))
